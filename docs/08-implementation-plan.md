...
**4a — Fill (submitter):** `/f/[key]` fetches the published config through
TanStack Query (React Query) in a leaf Client Component + `SchemaForm` renderer
(rhf + shared Ajv); field widgets; loading / error / success; `422` details mapped
onto fields.
...
- Frontend API state uses React Query only; see
  [12-frontend-backend-todo.md](./12-frontend-backend-todo.md) for the query-key
  plan and remaining backend connection checklist.
