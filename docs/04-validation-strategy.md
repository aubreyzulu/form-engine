...
- **`allErrors: true`** so the user sees *every* problem at once, not one at a time.
...
| On `POST` submission | API (server) | **Authoritative.** Validate `data` against the *published version's* schema before persisting. Reject with `422` + errors. |
...
JSON Schema covers the *rules*; `uiSchema` covers *presentation* (which widget,
...
2. **`validateSupportedFields`** — every field uses a type/format the engine can
   ...
   | `boolean` | — |
   | `array` | `items` must be `string` + `enum` (multi-choice) |
