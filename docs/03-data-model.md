...
as immutable** — edits create a *new* version, never mutate an existing one.
...
`apps/api/prisma/seed.ts` publishes two example forms so the hosted URL is usable
immediately: **Beneficial Ownership Declaration** (text/select/number/boolean/date —
exercises required, min/max, enum, email & date formats) and a simpler **Contact
Request** (proving the same engine renders a second config with no code changes).
